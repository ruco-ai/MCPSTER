...
export function generateManifest(config: DeployConfig): CloudflareManifest {
  return {
    name: config.name,
    main: 'worker.js',
...
export function generateWorkerShim(config: DeployConfig): string {
...
export function manifestToToml(manifest: CloudflareManifest): string {
  const lines = [
    `name = "${manifest.name}"`,
    `main = "${manifest.main}"`,
...
export async function deploy(config: DeployConfig): Promise<DeployResult> {
  ...
  const result = JSON.parse(output) as { url?: string; deployment_url?: string }
